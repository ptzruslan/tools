...
3. **Переключиться на нового пользователя**:
   ...
   ```
---
4. **Генерация ключа и настройка входа**:
   #### Генерация ключа:
   ```bash
   ssh-keygen  # Не меняйте путь сохранения ключа! Также можно создать пароль для ключа.
   ```
   #### Отправка ключа на сервер:
     - Windows OS: (не забудьте изменить имя и ip адрес)
    ```bash
   Get-Content "$env:USERPROFILE\.ssh\id_rsa.pub" | ssh <имя_пользователя>@<ip_сервера> "mkdir -p ~/.ssh; cat >> ~/.ssh/authorized_keys; chmod 700 ~/.ssh; chmod 600 ~/.ssh/authorized_keys"
   ```
